...
def GetRatio():
    ...
    for i in range(len(WIFIAPTag_List)):
        ...
        Error_List.append(Error.mean(axis=1))
        t2 = time.time()
    ...
    Save_Obj(Error_List,'Error_List')
    return Ratio_Dict , Error_List
...
def Predict(TrainTime,PredictTime,Ratio_Dict,Judge):
    count=0
    tag = WIFIAPTag_List[0]
    if Judge[count] == 1:
        try:        
            Predict = Do_ARMA(tag,TrainTime,PredictTime,4,2)
            if np.isnan(Predict.values).any():
                print 'Predict Nan in ARMA'
                Predict = Imitate2(tag,TrainTime,PredictTime)
        except:
            print 'Predict ARMA Failed'
            Predict = Imitate2(tag,TrainTime,PredictTime)
    else:        
...
if __name__ == '__main__':
    #Ratio_Dict ,Error_List= GetRatio()
    # load result
    Ratio_Dict = Import_Obj('Ratio_Dict')
    Error_List = Import_Obj('Error_List') 
    E = pd.DataFrame(Error_List)
    E.columns = ['im1','im2','arma']
    Judge = [1 if (e1>600 and e1 > e3) else 0 for e1,e3 in zip(E.im1,E.arma)]
    ...
    Predict(TrainTime,PredictTime,Ratio_Dict,Judge)
